...
function Visual.Popup(title,message,picture)
    if waveProperties.frameWork:lower() == "qbcore" then
        TriggerEvent("QBCore:Notify",message)
    else
        if waveProperties.Notifications.useCustom then
            waveProperties.Notifications.customNotificationFunction(title,message)
        else
            AddTextEntry('waveNotification', "→ "..message)
            BeginTextCommandThefeedPost('waveNotification')
            ThefeedNextPostBackgroundColor(140)
            EndTextCommandThefeedPostMessagetext(picture, picture, true, 2, GetPlayerName(PlayerId()), title)
...
function Visual.GTAMessage(text,desc,effect,duration)
    local count = 0
    RageUI.PlaySound("DLC_HEIST_HACKING_SNAKE_SOUNDS","Goal")
...
RegisterNetEvent("ayznnn_ui:drawNotification")
AddEventHandler("ayznnn_ui:drawNotification", function(array)
    Visual.Popup(array.title,array.message,array.picture)
end)